...
from AcceLEGOrator import Drift, Quadrupole, Dipole, ThinRfCavity, Constants, Physics
...
class MADXParser:
    ...
    def __init__(self):
        ...
        self.__rfcavity_pattern = '(.*):rfcavity,(.*)=(.*),(.*)=(.*),(.*)=(.*),(.*)=(.*);'
        
        # don't count name and type --> len - 2
        self.__nRfCavity = 2 * (len(self.__rfcavity) - 2)
    ...
    def parse(self, fn):
        ...
        with open(fn, 'r') as f:
            for line in f:
                ...
                elif 'rfcavity' in line:
                    
                    obj = re.match(self.__rfcavity_pattern, line)
                    
                    # first tag is name
                    self.__rfcavity['name'] = obj.group(1)
                    
                    for i in range(2, self.__nRfCavity + 2, 2):
                        if obj.group(i) in self.__rfcavity:
                            self.__rfcavity[obj.group(i)] = float(obj.group(i+1))
                        else:
                            raise MADXInputError('RFCavity',
                                                 'Line ' + str(nLine) + ': Parameter ' +
                                                 "'" + obj.group(i) + "'" +
                                                 ' does not exist for dipole.')
                        
                    self.__elements.append( self.__rfcavity.copy() )
    ...
    def __str__(self):
        ...
        if self.__lattice:
            ...
            # drift, dipole, quadrupole, rfcavity
            nTypes = [0, 0, 0, 0]
                ...
                for e in self.__elements:
                    if elem == e['name']:
                        if e['type'] == 'drift':
                            ...
                            beamline.append(ThinRfCavity(e['volt'], e['freq'], e['lag']))
                        else:
